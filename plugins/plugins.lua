...
local function list_plugins(only_enabled, msg)
  ...
  text = "\n_🔃 ᴀʟʟ ᴘʟᴜɢɪɴs ʀᴇʟᴏᴀᴅᴇᴅ_\n\n"..nact.." *✔️ ᴘʟᴜɢɪɴs ᴇɴᴀʙʟᴇᴅ*\n"..nsum.." *📂 ᴘʟᴜɢɪɴs ɪɴsᴛᴀʟʟᴇᴅ*\n"
...
local function enable_plugin( plugin_name, msg )
  ...
  if plugin_enabled(plugin_name) then
    local text = '<b>'..plugin_name..'</b> <i>ɪs ᴇɴᴀʙʟᴇᴅ.</i>'
	tdcli.sendMessage(msg.to.id, msg.id_, 1, text, 1, 'html')
	return
  end
...
local function disable_plugin( name, msg )
  ...
  -- Check if plugin is enabled
  if not k then
    local text = '<b>'..name..'</b> <i>ɴᴏᴛ ᴇɴᴀʙʟᴇᴅ.</i>'
	tdcli.sendMessage(msg.to.id, msg.id_, 1, text, 1, 'html')
	return
  end
...
local function disable_plugin_on_chat(receiver, plugin, msg)
  if not plugin_exists(plugin) then
    return "_ᴘʟᴜɢɪɴ ᴅᴏᴇsɴ'ᴛ ᴇxɪsᴛs_"
  end
  ...
  save_config()
  local text = '<b>'..plugin..'</b> <i>ᴅɪsᴀʙʟᴇᴅ ᴏɴ ᴛʜɪs ᴄʜᴀᴛ.</i>'
  tdcli.sendMessage(msg.to.id, msg.id_, 1, text, 1, 'html')
end

local function reenable_plugin_on_chat(receiver, plugin, msg)
  if not _config.disabled_plugin_on_chat then
    return 'ᴛʜᴇʀᴇ ᴀʀᴇ ɴᴏᴛ ᴀɴʏ ᴅɪsᴀʙʟᴇᴅ ᴘʟᴜɢɪɴs.'
  end

  if not _config.disabled_plugin_on_chat[receiver] then
    return 'ᴛʜᴇʀᴇ ᴀʀᴇ ɴᴏᴛ ᴀɴʏ ᴅɪsᴀʙʟᴇᴅ ᴘʟᴜɢɪɴs ғᴏʀ ᴛʜɪs ᴄʜᴀᴛ.'
  end
  ...
  save_config()
  local text = '<b>'..plugin..'</b> <i>ɪs ᴇɴᴀʙʟᴇᴅ ᴀɢᴀɪɴ.</i>'
  tdcli.sendMessage(msg.to.id, msg.id_, 1, text, 1, 'html')
end
...
local Clang = redis:get(Chash)
  ...
  if matches[2] == '-' and is_sudo(msg) then --after changed to moderator mode, set only sudo
    if matches[3] == 'plugins' then
		return 'ᴛʜɪs ᴘʟᴜɢɪɴ ᴄᴀɴ ɴᴏᴛ ʙᴇ ᴅɪsᴀʙʟᴇᴅ.'
    end
