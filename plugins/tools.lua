local SUDO = 284568421 -- put Your ID here! <===
...
if ((matches[1]:lower() == "clear cache" and not Clang) or (matches[1] == "پاک کردن حافظه" and Clang)) then
...
if ((matches[1]:lower() == "visudo" and not Clang) or (matches[1] == "سودو" and Clang)) then
...
if ((matches[1]:lower() == "desudo" and not Clang) or (matches[1] == "حذف سودو" and Clang)) then
if not matches[2] and msg.reply_id then
    tdcli_function ({
      ID = "GetMessage",
      chat_id_ = msg.to.id,
  ...
  if matches[2] and not string.match(matches[2], '^%d+$') then
   tdcli_function ({
      ...
      username_ = matches[2]
		...
		if ((matches[1]:lower() == "config" and not Clang) or (matches[1] == "پیکربندی" and Clang)) and is_admin(msg) then
...
if is_sudo(msg) then
		...
		if ((matches[1]:lower() == 'leave' and not Clang) or (matches[1] == "خروج" and Clang)) and matches[2] then
			if lang then
	...
	if msg.to.type == 'channel' or msg.to.type == 'chat' then
		if ((matches[1]:lower() == 'charge' and not Clang) or (matches[1] == "شارژ" and Clang)) and matches[2] and not matches[3] and is_sudo(msg) then
	...
	if ((matches[1]:lower() == 'check' and not Clang) or (matches[1] == "اعتبار" and Clang)) and is_mod(msg) and matches[2] then
		if string.match(matches[2], '^-%d+$') then
...
if ((matches[1]:lower() == "adminprom" and not Clang) or (matches[1] == "ادمین" and Clang)) and is_sudo(msg) then
if not matches[2] and msg.reply_id then
    tdcli_function ({
      ID = "GetMessage",
      chat_id_ = msg.to.id,
...
if ((matches[1]:lower() == "admindem" and not Clang) or (matches[1] == "حذف ادمین" and Clang)) and is_sudo(msg) then
if not matches[2] and msg.reply_id then
    tdcli_function ({
      ID = "GetMessage",
      chat_id_ = msg.to.id,
...
if ((matches[1]:lower() == 'creategroup' and not Clang) or (matches[1] == "ساخت گروه" and Clang)) and is_admin(msg) then
...
if ((matches[1]:lower() == 'createsuper' and not Clang) or (matches[1] == "ساخت سوپرگروه" and Clang)) and is_admin(msg) then
local text = matches[2]
...
if ((matches[1]:lower() == 'tosuper' and not Clang) or (matches[1] == "تبدیل به سوپرگروه" and Clang)) and is_admin(msg) then
local text = matches[2]
tdcli.createNewChannelChat(text, 1, '@KorenaTeam', (function(b, d) tdcli.addChatMember(d.id_, msg.from.id, 0, dl_cb, nil) end), nil)
   if not lang then 
...
if ((matches[1]:lower() == 'import' and not Clang) or (matches[1] == "ورود لینک" and Clang)) and is_admin(msg) then
...
if ((matches[1]:lower() == 'setbotname' and not Clang) or (matches[1] == "تغییر نام ربات" and Clang)) and is_sudo(msg) then
...
if ((matches[1]:lower() == 'setbotusername' and not Clang) or (matches[1] == "تغییر یوزرنیم ربات" and Clang)) and is_sudo(msg) then
...
if ((matches[1]:lower() == 'delbotusername' and not Clang) or (matches[1] == "حذف یوزرنیم ربات" and Clang)) and is_sudo(msg) then
...
if ((matches[1]:lower() == 'markread' and not Clang) or (matches[1] == "" and Clang)) and is_sudo(msg) then
...
if ((matches[1]:lower() == 'bc' and not Clang) or (matches[1] == "ارسال" and Clang)) and is_admin(msg) then
		local text = matches[2]
...
if ((matches[1]:lower() == 'sudolist' and not Clang) or (matches[1] == "لیست سودو" and Clang)) and is_sudo(msg) then
return sudolist(msg)
    end
...
if ((matches[1]:lower() == 'korena' and not Clang) or (matches[1] == "کورنا" and Clang)) then
return tdcli.sendMessage(msg.to.id, msg.id, 1, _config.info_text, 1, 'md')
    end
if ((matches[1]:lower() == 'adminlist' and not Clang) or (matches[1] == "لیست ادمین" and Clang)) and is_admin(msg) then
return adminlist(msg)
    end
     if ((matches[1]:lower() == 'leave' and not Clang) or (matches[1] == "خروج" and Clang)) and is_admin(msg) then
  tdcli.changeChatMemberStatus(msg.to.id, our_id, 'Left', dl_cb, nil)
   end
     if ((matches[1]:lower() == 'autoleave' and not Clang) or (matches[1] == "خروج خودکار" and Clang)) and is_admin(msg) then
...
if matches[1]:lower() == "helptools" and not Clang and is_mod(msg) then
if not lang then
text = [[

_Sudoer And Admins Korena Bot Help :_
...
text = [[
_راهنمای ادمین و سودو های ربات کورنا:_
