local function run(msg, matches)
 ...
 if matches[1]:lower() == 'setmaster' and is_admin(msg) then
  ...
  if chk then
   ...
   return '_ᴍᴀsᴛᴇʀ ɪᴅ ᴡᴀs ʀᴇᴘʟᴀᴄᴇᴅ._'
  else 
   ldata[tostring(msg.chat_id_)]["master"] = {id = idmas}
   save_data(_config.moderation.data, ldata)
   return '_ᴍᴀsᴛᴇʀ ɪᴅ sᴀᴠᴇᴅ._'
   end
 end
  ...
  if master then
   ...
   function id_cb(arg, data)
    ...
    tdcli.sendMessage(msg.chat_id_, 0, 1,  '*ʏᴏᴜʀ ʀᴇᴘᴏʀᴛ ᴡᴀs sᴇɴᴛ ᴛᴏ* [ '..master..' ]', 1, 'md')
    tdcli.sendMessage(master, 0, 1, '[ <code>'..msg.sender_user_id_..'</code> ] '..user_name..' <b>sᴇɴᴅ ᴛᴏ ʏᴏᴜ ᴀ ʀᴇᴘᴏʀᴛ:</b>', 1, 'html')
    tdcli.forwardMessages(master, msg.chat_id_,{[0] = msg.reply_to_message_id_}, 0,dl_cb,nil)
   end
  else
   return '_ᴍᴀsᴛᴇʀ ɪᴅ ʜᴀs ɴᴏᴛ ʙᴇᴇɴ sᴇᴛ!_'
  end
