...
--------------------------------
 ...
 local text = matches[2]
  ...
  local file = download_to_file(url,'Korena-VC.mp3')
 				tdcli.sendDocument(msg.to.id, 0, 0, 1, nil, file, '', dl_cb, nil)
   end
end
...
if matches[1]:lower() == 'ping' then
    ...
    return tdcli.sendDocument(msg.chat_id_, 0, 0, 1, nil, './data/ping.gif', text, dl_cb, nil)
end
--------------------------------
if matches[1]:lower() == "helpfun" and not Clang then
local hash = "gp_lang:"..msg.to.id
local lang = redis:get(hash)
if not lang then
helpfun_en = [[
_Korena Fun Help Commands:_
