...
if ((matches[1]:lower() == "add" and not Clang) or (matches[1] == "افزودن" and Clang)) then
return modadd(msg)
end
if ((matches[1]:lower() == "rem" and not Clang) or (matches[1] == "حذف گروه" and Clang)) then
return modrem(msg)
end
if not data[tostring(msg.to.id)] then return end
if (matches[1]:lower() == "id" and not Clang) or (matches[1] == "ایدی" and Clang) then
...
local function getpro(arg, data)
   ...
   if data.photos_[0] then
       ...
       if not lang then
            tdcli.sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, data.photos_[0].sizes_[1].photo_.persistent_id_,'🎗 ɴᴀᴍᴇ : '..(msg.from.print_name or NoNaMe)..'\n🎗 ᴜsᴇʀɴᴀᴍᴇ : @'..(msg.from.username or NoUserNaMe)..'\n🎗 ᴜsᴇʀ ɪᴅ : '..msg.from.id..'\n🎗 ᴄʜᴀᴛ ɪᴅ : '..msg.to.id,dl_cb,nil)
       elseif lang then
...
if ((matches[1]:lower() == "pin" and not Clang) or (matches[1] == "سنجاق" and Clang)) and is_mod(msg) and msg.reply_id then
...
if ((matches[1]:lower() == "setowner" and not Clang) or (matches[1] == 'مالک' and Clang)) and is_admin(msg) then
...
if ((matches[1]:lower() == "remowner" and not Clang) or (matches[1] == "حذف مالک" and Clang)) and is_admin(msg) then
if not matches[2] and msg.reply_id then
    tdcli_function ({
      ID = "GetMessage",
      chat_id_ = msg.to.id,
...
if ((matches[1]:lower() == "promote" and not Clang) or (matches[1] == "مدیر" and Clang)) and is_owner(msg) then
if not matches[2] and msg.reply_id then
    tdcli_function ({
      ID = "GetMessage",
      chat_id_ = msg.to.id,
...
if ((matches[1]:lower() == "demote" and not Clang) or (matches[1] == "حذف مدیر" and Clang)) and is_owner(msg) then
if not matches[2] and msg.reply_id then
 tdcli_function ({
      ID = "GetMessage",
      chat_id_ = msg.to.id,
...
if ((matches[1]:lower() == "lock" and not Clang) or (matches[1] == "قفل" and Clang)) and is_mod(msg) then
local target = msg.to.id
...
if ((matches[1]:lower() == "unlock" and not Clang) or (matches[1] == "باز کردن" and Clang)) and is_mod(msg) then
local target = msg.to.id
if ((matches[2] == "link" and not Clang) or (matches[2] == "لینک" and Clang)) then
...
if ((matches[1]:lower() == "mute" and not Clang) or (matches[1] == "بیصدا" and Clang)) and is_mod(msg) then
local target = msg.to.id
...
if ((matches[1]:lower() == "unmute" and not Clang) or (matches[1] == "باصدا" and Clang)) and is_mod(msg) then
local target = msg.to.id
if ((matches[2] == "all" and not Clang) or (matches[2] == "همه" and Clang)) then
...
if ((matches[1]:lower() == "gpinfo" and not Clang) or (matches[1] == "اطلاعات گروه" and Clang)) and is_mod(msg) and msg.to.type == "channel" then
...
if ((matches[1]:lower() == 'newlink' and not Clang) or (matches[1] == "لینک جدید" and Clang)) and is_mod(msg) then
 ...
 tdcli.exportChatInviteLink(msg.to.id, callback_link, nil)
		...
		if ((matches[1]:lower() == 'setlink' and not Clang) or (matches[1] == "تنظیم لینک" and Clang)) and is_owner(msg) then
   ...
   local is_link = msg.text:match("^([https?://w]*.?telegram.me/joinchat/%S+)$") or msg.text:match("^([https?://w]*.?t.me/joinchat/%S+)$")
    ...
    if ((matches[1]:lower() == 'link' and not Clang) or (matches[1] == "لینک" and Clang)) and is_mod(msg) then
    ...
    if ((matches[1]:lower() == 'linkpv' and not Clang) or (matches[1] == "لینک خصوصی" and Clang)) and is_mod(msg) then
      local linkgp = data[tostring(chat)]['settings']['linkgp']
      if not linkgp then
      if not lang then
        return "_First create a link for group with using_ /newlink\n_If bot not group creator set a link with using_ /setlink"
  ...
  if ((matches[1]:lower() == "setrules" and not Clang) or (matches[1] == "تنظیم قوانین" and Clang)) and matches[2] and is_mod(msg) then
  ...
  if ((matches[1]:lower() == "rules" and not Clang) or (matches[1] == "قوانین" and Clang)) then
...
if ((matches[1]:lower() == "res" and not Clang) or (matches[1] == "کاربری" and Clang)) and matches[2] and is_mod(msg) then
    tdcli_function ({
      ID = "SearchPublicChat",
      username_ = matches[2]
    }, action_by_username, {chat_id=msg.to.id,username=matches[2],cmd="res"})
  end
if ((matches[1]:lower() == "whois" and not Clang) or (matches[1] == "شناسه" and Clang)) and matches[2] and is_mod(msg) then
tdcli_function ({
    ID = "GetUser",
    user_id_ = matches[2],
  ...
  if ((matches[1]:lower() == 'setfloodtime' and not Clang) or (matches[1] == "تنظیم زمان بررسی" and Clang)) and is_mod(msg) then
    ...
    return "_حداکثر زمان بررسی پیام های مکرر تنظیم شد به :_ *[ "..matches[2].." ]*"
		...
		if ((matches[1]:lower() == 'clean' and not Clang) or (matches[1] == "پاک کردن" and Clang)) and is_owner(msg) then
			...
			if ((matches[2]:lower() == 'filterlist' and not Clang) or (matches[2] == "لیست فیلتر" and Clang)) then
     ...
     if not lang then
       ...
       if not lang then
			...
			if ((matches[2]:lower() == 'rules' and not Clang) or (matches[2] == "قوانین" and Clang)) then
				if not data[tostring(chat)]['rules'] then
            if not lang then
			...
			if ((matches[2]:lower() == 'welcome' and not Clang) or (matches[2] == "خوشامد" and Clang)) then
			...
			if ((matches[2]:lower() == 'about' and not Clang) or (matches[2] == "درباره" and Clang)) then
...
if ((matches[1]:lower() == "setname" and not Clang) or (matches[1] == "تنظیم نام" and Clang)) and matches[2] and is_mod(msg) then
local gp_name = matches[2]
tdcli.changeChatTitle(chat, gp_name, dl_cb, nil)
end
  if ((matches[1]:lower() == "setabout" and not Clang) or (matches[1] == "تنظیم درباره" and Clang)) and matches[2] and is_mod(msg) then
  ...
  if ((matches[1]:lower() == "about" and not Clang) or (matches[1] == "درباره" and Clang)) and msg.to.type == "chat" then
 if not data[tostring(chat)]['about'] then
     if not lang then
  ...
  if ((matches[1]:lower() == 'filter' and not Clang) or (matches[1] == "فیلتر" and Clang)) and is_mod(msg) then
    return filter_word(msg, matches[2])
  end
  if ((matches[1]:lower() == 'unfilter' and not Clang) or (matches[1] == "حذف فیلتر" and Clang)) and is_mod(msg) then
    return unfilter_word(msg, matches[2])
  end
  if ((matches[1]:lower() == 'filterlist' and not Clang) or (matches[1] == "لیست فیلتر" and Clang)) and is_mod(msg) then
    return filter_list(msg)
  end
if ((matches[1]:lower() == "settings" and not Clang) or (matches[1] == "تنظیمات" and Clang)) then
return group_settings(msg, target)
end
if ((matches[1]:lower() == "mutelist" and not Clang) or (matches[1] == "لیست بیصدا" and Clang)) then
return mutes(msg, target)
end
if ((matches[1]:lower() == "modlist" and not Clang) or (matches[1] == "لیست مدیران" and Clang)) then
return modlist(msg)
end
if ((matches[1]:lower() == "ownerlist" and not Clang) or (matches[1] == "لیست مالکان" and Clang)) and is_owner(msg) then
return ownerlist(msg)
end
if ((matches[1]:lower() == "whitelist" and not Clang) or (matches[1] == "لیست سفید" and Clang)) and not matches[2] and is_mod(msg) then
...
if (matches[1]:lower() == 'زبان' and Clang) and is_owner(msg) then
local hash = "gp_lang:"..msg.to.id
...
if (matches[1]:lower() == "help" and not Clang) and is_mod(msg) then
if not lang then
text = [[
*Korena Bot Commands:*
...
--------------------- Welcome -----------------------
	if ((matches[1]:lower() == "welcome" and not Clang) or (matches[1] == "خوشامد" and Clang)) and is_mod(msg) then
	...
	if ((matches[1]:lower() == "setwelcome" and not Clang) or (matches[1] == "تنظیم خوشامد" and Clang)) and matches[2] and is_mod(msg) then
